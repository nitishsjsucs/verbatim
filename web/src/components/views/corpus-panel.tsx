    ...
    if (!corpus || corpus.documents.length === 0) {
        return (
            <div className={cn("flex flex-col items-center justify-center h-full text-center p-4", className)}>
                ...
                <p className="text-xs text-muted-foreground">No documents in corpus</p>
    ...
    return (
        <div className={cn("flex flex-col h-full", className)}>
            ...
            <div className="h-11 border-b border-border flex items-center px-4 shrink-0 gap-2">
                ...
                <span className="text-xs font-medium text-foreground">Corpus</span>
                    ...
                    return (
                        <div key={doc.doc_id} className="border border-border/30 rounded-lg overflow-hidden">
                            ...
                            >
                                ...
                                {rels.length > 0 && (
                                    <span className="text-xs px-1.5 py-0.5 bg-primary/10 text-primary rounded-full shrink-0">
                                        {rels.length}
                                    </span>
                                )}
                            </button>
                            ...
                            {isExpanded && (
                                <div className="px-3 pb-3 space-y-3 border-t border-border/20">
                                    ...
                                    {doc.top_topics.length > 0 && (
                                        <div>
                                            <p className="text-xs font-medium text-muted-foreground mb-1 uppercase tracking-wider">Topics</p>
                                            <div className="flex flex-wrap gap-1">
                                                {doc.top_topics.slice(0, 12).map((topic, i) => (
                                                    <span key={i} className="px-1.5 py-0.5 bg-muted/50 text-muted-foreground rounded text-xs">
                                                        {topic}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    {rels.length > 0 && (
                                        <div>
                                            <p className="text-xs font-medium text-muted-foreground mb-1.5 uppercase tracking-wider">Relationships</p>
                                                ...
                                                {rels.map((rel, i) => {
                                                    ...
                                                    return (
                                                        <div key={i} className="flex items-start gap-2 text-xs">
                                                            <span className={cn("inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium shrink-0 mt-0.5", relConfig.color)}>
                                                                {relConfig.icon}
                                                                {relConfig.label}
                                                            </span>
