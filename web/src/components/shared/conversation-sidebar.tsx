    ...
    return (
        <div className="w-56 border-r border-border flex flex-col shrink-0 bg-sidebar">
            ...
            <div className="h-11 border-b border-border flex items-center px-3 justify-between shrink-0">
                <span className="text-sm-minus font-medium text-foreground">{title}</span>
            ...
            <div className="flex-1 overflow-y-auto py-1">
                {conversations.length === 0 && (
                    <div className="px-3 py-8 text-center">
                        ...
                        <p className="text-2xs text-muted-foreground/40">{emptyText}</p>
                    </div>
                )}
                {conversations.map((conv) => (
                    ...
                    >
                        ...
                        <div className="flex-1 min-w-0">
                            <p className="text-xs-plus font-medium truncate">
                                {conv.title || conv.last_message_preview || fallbackName}
                            </p>
                            <p className="text-3xs text-muted-foreground/50 mt-0.5">
                        ...
                        {confirmDeleteId === conv.conv_id ? (
                            <div className="flex items-center gap-0.5 shrink-0" onClick={(e) => e.stopPropagation()}>
                                <button
                                    ...
                                    className="text-3xs text-red-400 hover:text-red-300 px-1 py-0.5 rounded bg-red-400/10"
                                >
                                    Del
                                </button>
                                <button
                                    onClick={() => setConfirmDeleteId(null)}
                                    className="text-3xs text-muted-foreground px-1 py-0.5"
                                >
                                    No
                                </button>
                            </div>
