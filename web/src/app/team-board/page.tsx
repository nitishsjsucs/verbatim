                            ...
                            <div className="grid grid-cols-2 gap-2 mb-2">
                                <div className="rounded-lg border border-border/40 bg-background/60 p-2">
                                    ...
                                    <p className="text-sm font-semibold tabular-nums text-blue-400">
                                        {(item.overall_likelihood_score ?? item.likelihood_score ?? 0) > 0
                                            ? (item.overall_likelihood_score ?? item.likelihood_score)
                                ...
                                <div className="rounded-lg border border-border/40 bg-background/60 p-2">
                                    ...
                                    <p className="text-sm font-semibold tabular-nums text-pink-400">
                                        {(item.overall_impact_score ?? item.impact_score ?? 0) > 0
                                            ? (item.overall_impact_score ?? item.impact_score)
                                            : <span className="text-muted-foreground/40 text-xs">—</span>}
                                    </p>
                                </div>
                                <div className="rounded-lg border border-border/40 bg-background/60 p-2">
                                    ...
                                    <p className="text-sm font-semibold tabular-nums text-orange-400">
                                        {(item.inherent_risk_score ?? 0) > 0
                                            ? item.inherent_risk_score!.toFixed(0)
                                            : <span className="text-muted-foreground/40 text-xs">—</span>}
                                    </p>
                                ...
                                <div className="rounded-lg border border-border/40 bg-background/60 p-2">
                                    ...
                                    <p className="text-sm font-semibold tabular-nums text-teal-400">
                                        {(item.overall_control_score ?? item.control_score ?? 0) > 0
                                            ? (item.overall_control_score ?? item.control_score)!.toFixed(1)
                                            : <span className="text-muted-foreground/40 text-xs">—</span>}
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-sm font-semibold tabular-nums text-foreground">
                                            {(item.residual_risk_score ?? 0) > 0
                                                ? item.residual_risk_score!.toFixed(1)
                                                : <span className="text-muted-foreground/40 text-xs">—</span>}
                                        </p>
                                    </div>
                                    {(() => {
